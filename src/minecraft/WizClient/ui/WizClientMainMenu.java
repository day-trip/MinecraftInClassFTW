...
public class WizClientMainMenu extends GuiScreen {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		// guiUtils2.drawRoundOutline(8, 7, (int) ((float)36 +
		// ufr.getWidth(mc.getSession().getUsername())), 23, 4f, 1.0f, color.getRGB());
		HeadUtil.drawPlayerHead(mc.getSession().getUsername(),
				this.width - ufr.getStringWidth(mc.getSession().getUsername()) - 40, 5, 16);
		ufr.drawString(mc.getSession().getUsername(),
				this.width - ufr.getStringWidth(mc.getSession().getUsername()) - 20, 7, -1);

		if (mc.getSession().getUsername() != null) {
			boolean hovering = mouseX >= this.width - ufr.getStringWidth(mc.getSession().getUsername()) - 47
					&& mouseX <= this.width + ufr.getWidth(mc.getSession().getUsername()) - 50 && mouseY <= 23;
			if (hovering) {
			...
			// guiUtils2.drawRoundOutline(this.width -
			// ufr.getStringWidth(mc.getSession().getUsername()) - 47, 3, (int) ((float)
			// this.width - ufr.getWidth(mc.getSession().getUsername())), 23, 4f, 1.0f,
			// color.getRGB());
			guiUtils2.drawRoundOutline(this.width - ufr.getStringWidth(mc.getSession().getUsername()) - 45, 3,
					this.width - 5, 25, 4f, 1.0f, color.getRGB());
		}
		...
		ufr.drawString(copyright, this.width - this.fontRendererObj.getStringWidth(copyright) + 27, this.height - 13,
				-1);
	...
	public void drawlogo() {
		...
		Wr.pos((double) 110 - 120, (double) this.height / 2 - 20, (double) this.zLevel).tex((double) 0, (double) 1)
				.endVertex();
		Wr.pos((double) 110 + 120, (double) this.height / 2 - 20, (double) this.zLevel).tex((double) 1, (double) 1)
				.endVertex();
