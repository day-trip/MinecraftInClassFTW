...
public class MicrosoftAuthenticator
{
    ...
    public static void refreshUser(Minecraft mc) {
    	System.out.println("Attempting to refresh user!");
    	if (mc.gameSettings.refreshToken != null && mc.gameSettings.refreshToken.length() > 0) {
    		MicrosoftAuthenticator authenticator = new MicrosoftAuthenticator();
    		try {
    			MicrosoftAuthResult acc = authenticator.loginWithRefreshToken(mc.gameSettings.refreshToken);
    	    	mc.gameSettings.refreshToken = acc.getRefreshToken();
    			mc.session = new Session(acc.getProfile().getName(), acc.getProfile().getId(), acc.getAccessToken(), "legacy");
    		} catch (MicrosoftAuthenticationException e) {
    			System.out.println("Refresh login failed!");
    			e.printStackTrace();
    		}
    	}
    }
    ...
    public MicrosoftAuthResult loginWithWebview() throws MicrosoftAuthenticationException
    {
        try {
        	...
        	System.out.println("RELLY BAD #3");
    ...
    protected String extractValue(String url, String key) throws MicrosoftAuthenticationException
    {
        ...
        try {
        	...
        	System.out.println("RELLY BAD #2");
            throw new MicrosoftAuthenticationException(e);
        }
    }
